...
| `model_wait_recovered_comment_failed` | publishing the model-wait recovery comment failed (the run stays recoverable) |
...
| `empty_next_step` | a terminal progress comment had an empty next-step blob |
| `activity_snapshot_failed` | reading the live Pi activity snapshot failed (best-effort; the task continues) |
...
| `failure_reporting_failed` | publishing the delivery failure comment failed (the original failure still stands) |
...
| `opened_pr_scene_comment_failed` | publishing the opened-PR scene comment failed (PR stays ai-pr-opened) |
