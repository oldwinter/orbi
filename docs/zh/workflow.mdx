...
`auto_create_release_ticket`（默认 `false`）同属这类宿主键。发版关闭
当前 Milestone 后，idle 要么改写宿主 `orbi.toml`，要么停在待确认。
发版之前，若 Milestone 已做完却没有 `ai-release` 票，建票开关（默认
关闭）会幂等开一张，武装仍走原有 idle 路径。两个键都不能写进仓库文件。
