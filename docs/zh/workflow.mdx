...
- `external_commits` —— 不在引擎自己推送历史里的 commit 数（开 PR 时的
  ...
  head），减去记录的最后一个引擎推送 head。
- `external_commits=unknown` —— 推送历史无法证实（重建的 worktree 丢了
  run-state 记录，或记录的 head 不是合并 head 的祖先）。数不出来的历史
  绝不写成 `0`。

引擎推送 head 只在两个点记入 task worktree 的 run state（`pushed_head`，
Issue #219 文件）——交付推送与 verdict 覆盖的 review/fix 推送——同一
worktree 的接续都能恢复这份历史；全新的 worktree 从无记录开始，退化为
`unknown`。
