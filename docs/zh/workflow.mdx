...
`auto_next_milestone`（默认 `true`）与
`warn_on_dangling_milestone`（默认 `false`）同属这类宿主键：发版关闭
当前 Milestone 后，idle tick 要么改写宿主 `orbi.toml`
（`auto_next_milestone = true`），要么停在待确认（`false`）。告警
开关**不推进**，只发出 `active_milestone_dangling`，让空转可被检索。
`auto_create_release_ticket`（默认 `false`）是发版另一侧的同型宿主键：
Milestone 仍 open、`open_issues` 为 0、且没有 `ai-release` 票时，idle
tick 可按引擎模板建一张。这些键都不能写进仓库文件。
