...
| `auto_next_milestone` | 否 | `true` | 当前 Milestone 关闭且队列为空时是否自动推进。`false` 保持 `active_milestone` 不变，记录待确认告警并创建一个不带 `ai-ready` 或 `p0`、不会被领取的确认 Issue；人工推进后旧通知会自动关闭。`orbi milestone set` 在此为 true 时拒绝执行（Issue #933）：下个空闲 tick 会改写刚写入的值。非法值立即失败 |
...
| `auto_create_release_ticket` | 否 | `false` | 仅宿主的 idle 建票（Issue #856）：当 `active_milestone` 仍 **open**、`open_issues` 为 0、且该 Milestone 下没有任何 `ai-release` 票（open 或 closed）时，显式 `true` 按 `.github/release-ticket-template.md` 创建一张（`version` / `base_branch` / `scope_from_milestone`，指纹 `orbi-auto-release milestone=<title>`），dispatch 标签仍交给 `arm_release_ticket`。未设置 / `false` 与今天一致（只武装已有票）。非 Python 仓库无法推断 `version_file` 时不建票并留下 `release_ticket_create_skipped`。非法值让配置加载 fail fast |
