...
PyPI 发布包名为 `orbi-cli`（Issue #874）；安装后的命令仍是 `orbi`：

```bash
python3 -m pip install orbi-cli
orbi --version
orbi --help
```
...
（`python3 -m pip install --upgrade orbi-cli` /
`uv tool install --reinstall orbi-cli`；卸载用 `uv tool uninstall orbi-cli`）。
...
调度器 user units）；机器是否就绪用只读前置条件门禁 `orbi check` 验证
