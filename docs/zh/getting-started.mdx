...
| 症状 | 证据 | 修复 |
|---|---|---|
| 启动在任何 slot/claim 前失败 | journal `API key for provider ... referenced missing environment variable GROQ_API_KEY`（只报变量**名**） | 把 key 值放进你的 `.orbi/env`（第 4 步），并确认已安装 unit 的 `EnvironmentFile` 指向它；手工 tick 则在 shell 里 export 该变量 |
