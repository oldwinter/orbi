...
系统 Python 低于 3.14 时，安装会以 resolver 消息
`orbi-cli==0.5.6 depends on Python>=3.14` 失败；修复方式是选择兼容的
解释器——给 `uv tool install` 加 `--python 3.14`（uv 自动供应），或在
Python ≥ 3.14 构建的环境里运行 pip。
