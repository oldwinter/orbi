...
`pip install orbi` and `uv tool install orbi` currently fail with a
registry "not found" error. The supported installation paths are the
ones this page already carries: the one-line installer above (it clones
the source and installs the editable CLI for you) and the editable
checkout install in step 2. This section returns with the real commands
only after a release is actually published.
