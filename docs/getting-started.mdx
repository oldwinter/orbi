...
The CLI is **not published on PyPI yet** — the v0.5.5 publication failed
on the PyPI-side trusted-publisher setup
([Issue #852](https://github.com/orbi-build/orbi/issues/852)), so
`pip install orbi-cli` and `uv tool install orbi-cli` currently fail with
a registry "not found" error. The distribution name is `orbi-cli`
(Issue #874); the installed command stays `orbi`. The supported
installation paths are the ones this page already carries: the one-line
installer above (it clones the source and installs the editable CLI for
you) and the editable checkout install in step 2. This section returns
with the real commands only after a release is actually published.
...
selects one — Ubuntu 24.04 ships Python 3.12, so pinning its system
interpreter exits 1 with a dependency resolution failure:
