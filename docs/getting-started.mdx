...
required. The PyPI distribution is named `orbi-cli` (Issue #874); the
installed command stays `orbi`:
...
upgraded by reinstalling (`python3 -m pip install --upgrade orbi-cli` /
`uv tool install --reinstall orbi-cli`; `uv tool uninstall orbi-cli`
removes it). The editable checkout install below
...
that deployment layout (config, deploy home, scheduler user units); verify
