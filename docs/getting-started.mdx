...
[`orbi-cli`](https://pypi.org/project/orbi-cli/) — release 0.5.6,
sdist + wheel, requiring Python ≥ 3.14. The distribution name is
...
an explicit `--python 3.13` on the `uv tool install` call, or
`pip install orbi-cli` inside an activated older environment, fails
with the resolver message
`orbi-cli==0.5.6 depends on Python>=3.14`; the repair is to select a
compatible interpreter — point `uv tool install` at `--python 3.14`
(uv provisions it) or run pip inside an environment built on
Python ≥ 3.14.
