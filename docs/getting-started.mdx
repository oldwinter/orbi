...
| `auto_next_milestone` | no | `true` | Controls idle advancement after the active milestone closes and its queue is empty. `false` leaves `active_milestone` unchanged, logs a pending warning, and creates one confirmation Issue without `ai-ready` or `p0` so it cannot be picked up; stale notices are closed after a manual move. `orbi milestone set` refuses while this is true (Issue #933): the next idle tick would overwrite the value. Invalid values fail fast |
| `warn_on_dangling_milestone` | no | `false` | Host-only idle alarm (Issue #855) when `active_milestone` points at a **closed or missing** Milestone and the fresh-claim scans found nothing. `true` emits the structured `active_milestone_dangling active=<title> state=closed\|missing` journal event so a post-release stall is greppable. Unset / `false` matches today's journal (no extra line). Does not change `auto_next_milestone`, does not rewrite `active_milestone`, and does not change the three ready scans. Invalid values fail the config load fast |
| `auto_create_release_ticket` | no | `false` | Host-only idle create (Issue #856) when `active_milestone` is still **open**, its `open_issues` is 0, and the Milestone has no `ai-release` ticket (open or closed). `true` creates one Issue from `.github/release-ticket-template.md` (`version` / `base_branch` / `scope_from_milestone`, fingerprint `orbi-auto-release milestone=<title>`) and leaves dispatch-label arming to `arm_release_ticket`. Unset / `false` matches today (arm existing tickets only). Does not guess `version_file` for a non-Python checkout — skips with `release_ticket_create_skipped`. Invalid values fail the config load fast |
