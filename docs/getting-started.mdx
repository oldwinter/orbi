...
On a system Python below 3.14 the install fails with the resolver
message `orbi-cli==0.5.6 depends on Python>=3.14`; the repair is to
select a compatible interpreter — add `--python 3.14` to the
`uv tool install` call (uv provisions it), or run pip inside an
environment built on Python ≥ 3.14.
