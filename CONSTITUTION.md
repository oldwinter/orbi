...
| Surface | Definition |
|---|---|
| CLI | `orbi` (one Runner tick), `orbi add`, `orbi status`, `orbi session`, `orbi install-units`, `orbi sync-engine-source`, `orbi doctor`, `orbi check`, `orbi setup`, `orbi milestone` (`milestone set <title>`) |
