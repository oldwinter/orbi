...
def test_instance_names_are_generated_by_count():
    ...
    assert systemd_deploy.MAX_RUNNER_INSTANCES == 5
    assert systemd_deploy.timer_instances(count=1) == ("orbi@1.timer",)
    assert systemd_deploy.timer_instances(count=3) == (
        "orbi@1.timer", "orbi@2.timer", "orbi@3.timer",
    )
    assert systemd_deploy.service_instances(count=3) == (
        "orbi@1.service", "orbi@2.service", "orbi@3.service",
    )
    assert systemd_deploy.timer_instances("website", 2) == (
        "orbi-website@1.timer", "orbi-website@2.timer",
    )
    assert systemd_deploy.service_instances(None, 1) == ("orbi@1.service",)
...
def test_install_units_at_max_capacity_enables_every_instance(tmp_path):
    ...
    systemd_deploy.install_units(
        ...
        max_concurrency=systemd_deploy.MAX_RUNNER_INSTANCES,
        run_command=lambda command, **kwargs: calls.append(command) or "",
    )
    ...
    assert calls == [
        ...
        *[
            ["systemctl", "--user", "enable", "--now", f"orbi@{index}.timer"]
            for index in range(1, systemd_deploy.MAX_RUNNER_INSTANCES + 1)
        ],
        ["git", "rev-parse", "HEAD"],
    ]
...
def test_install_units_migrates_the_legacy_units_once(
    ...
    for instance in systemd_deploy.timer_instances(count=2):
        assert [
